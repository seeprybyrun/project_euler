...
# bottom right, by moving left, right, up, and down, is equal to 2297.
# 
# 131 673 234 103  18
# 201  96 342 965 150
# 630 803 746 422 111
...
for i in range(M):
    for j in range(N):
        if j > 0: # if not in first column, can move left
            neighbors[(i,j)] += [(i,j-1)]
...
print 'answer: {}'.format(answer) # 425185
print 'seconds elapsed: {}'.format(time.clock()-t0) # ~6.06s
